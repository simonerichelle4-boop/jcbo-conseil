...
function APropos() {
  ...
  const styles = {
    ...
    hero: { padding: isMobile ? '80px 20px 40px' : '140px 20px 80px', textAlign: 'center' },
    name: { 
    ...
    subtitle: { fontSize: isMobile ? '1rem' : '1.35rem', color: '#C9A445', fontWeight: '500' },
    
    // ✅ CORRECTION PRINCIPALE — 1 colonne sur mobile, 2 sur desktop
    contentGrid: { 
      display: 'grid', 
    ...
    skills: { padding: isMobile ? '40px 20px' : '80px 20px', backgroundColor: '#f9fafb' },
    skillsGrid: { 
      display: 'grid', 
    ...
    values: { padding: isMobile ? '40px 20px' : '100px 20px' },
    valuesGrid: { 
      display: 'grid', 
    ...
    cta: { 
      padding: isMobile ? '60px 20px' : '70px 40px', 
  ...
  return (
    <div>
      ...
      {/* Présentation */}
      <section style={{ padding: isMobile ? '20px 20px 60px' : '40px 20px 100px' }}>
        <div style={styles.container}>
          <div style={styles.contentGrid}>
            ...
            <div style={styles.textContent}>
              ...
              <p>Professionnel expérimenté avec 30 années de pratique dans les domaines du consulting, de la formation et du coaching. Expert en Business Development, coach et formateur professionnel en écoles supérieures de commerce en France.</p>
              <div style={styles.quote}>
                "Mon objectif est simple : transformer votre vision en résultats concrets et durables."
              </div>
              <div style={styles.textContent}>
                <p>Son expérience couvre la définition et la mise en oeuvre de stratégies commerciales pour plusieurs PME et TPE. Il organise également des séminaires, formations et évènements internationaux.</p>
              </div>
            </div>
          </div>
        </div>
